...
def validate_artifact_metadata(
    metadata: dict[str, Any], artifact_path: Path | None = None
) -> CatalogResult:
    ...
    for field_name in required_fields:
        if field_name not in metadata or metadata[field_name] is None:
            errors.append(
                ValidationError(
                    field=field_name, message=f"{field_name}: required field missing"
                )
            )
    ...
    if not str(metadata["topic"]).strip():
        errors.append(
            ValidationError(field="topic", message="topic: cannot be empty")
        )
    ...
    if not isinstance(keywords, list):
        errors.append(
            ValidationError(
                field="keywords", message="keywords: must be a list"
            )
        )
    elif len(keywords) < 3:
        errors.append(
    ...
    if not str(metadata["consensus"]).strip():
        errors.append(
            ValidationError(field="consensus", message="consensus: cannot be empty")
        )
